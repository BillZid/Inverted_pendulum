    ...
    def train(self, episode_num=500, episode_length=20000):
        ...
        while episode_count < episode_num:
            ...
            while episode_step < episode_length:
                ...
                index_new_action = self.value_to_index(
                    new_action, -self.umax, self.umax, self.n_actions)
                self.q_table[index_th, index_thdot, index_action] = lr * (
                    reward + gamma*self.q_table[index_new_th, index_new_thdot, index_new_action] - 
...
if __name__ == "__main__":
    agent = SARSA(n_th=300, n_thdot=300, n_actions=3, gamma=1)
    agent.load()  # load参数
    agent.train()  # 训练模型
    agent.save()
